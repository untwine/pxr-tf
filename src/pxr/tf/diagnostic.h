...
//
#ifndef PXR_TF_DIAGNOSTIC_H
#define PXR_TF_DIAGNOSTIC_H
...
/// \hideinitializer
#define TF_FUNC_NAME()                                 \
    pxr::ArchGetPrettierFunctionName(__ARCH_FUNCTION__, __ARCH_PRETTY_FUNCTION__)
...
#define TF_CODING_ERROR(...)                            \
    pxr::Tf_PostErrorHelper(TF_CALL_CONTEXT,            \
...
#define TF_FATAL_CODING_ERROR                                \
    pxr::Tf_DiagnosticHelper(TF_CALL_CONTEXT,           \
...
#define TF_CODING_WARNING(...)                   \
    pxr::Tf_PostWarningHelper(TF_CALL_CONTEXT,          \
        TF_DIAGNOSTIC_CODING_ERROR_TYPE, __VA_ARGS__)
...
#define TF_DIAGNOSTIC_WARNING                                \
    pxr::Tf_DiagnosticHelper(TF_CALL_CONTEXT.Hide(),           \
...
#endif // TF_WARN
#define TF_WARN(...)                                \
    pxr::Tf_PostWarningHelper(TF_CALL_CONTEXT, __VA_ARGS__)

#ifdef TF_STATUS
#undef TF_STATUS
#endif // TF_STATUS
#define TF_STATUS(...)                              \
    pxr::Tf_PostStatusHelper(TF_CALL_CONTEXT, __VA_ARGS__)

#ifdef TF_ERROR
#undef TF_ERROR
#endif // TF_ERROR
#define TF_ERROR(...)                              \
    pxr::Tf_PostErrorHelper(TF_CALL_CONTEXT, __VA_ARGS__)

#ifdef TF_QUIET_ERROR
#undef TF_QUIET_ERROR
#endif // TF_ERROR
#define TF_QUIET_ERROR(...)                              \
    pxr::Tf_PostQuietlyErrorHelper(TF_CALL_CONTEXT, __VA_ARGS__)

// See documentation above.
#define TF_VERIFY(cond, ...)                                                   \
    (ARCH_LIKELY(cond) ? true :                                                \
     pxr::Tf_FailedVerifyHelper(TF_CALL_CONTEXT, # cond,                       \
                           pxr::Tf_VerifyStringFormat(__VA_ARGS__)))
