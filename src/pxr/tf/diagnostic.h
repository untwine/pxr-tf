...
#include "pxr/tf/diagnosticHelper.h"
...
#define TF_CODING_ERROR(...)                            \
    TF_NS::Tf_PostErrorHelper(TF_CALL_CONTEXT,          \
...
#define TF_FATAL_CODING_ERROR                              \
    TF_NS::Tf_DiagnosticHelper(TF_CALL_CONTEXT,            \
...
#define TF_CODING_WARNING(...)                             \
    TF_NS::Tf_PostWarningHelper(TF_CALL_CONTEXT,           \
        TF_DIAGNOSTIC_CODING_ERROR_TYPE, __VA_ARGS__)
...
#define TF_DIAGNOSTIC_WARNING                              \
    TF_NS::Tf_DiagnosticHelper(TF_CALL_CONTEXT.Hide(),     \
...
#endif // TF_WARN
#define TF_WARN(...)                                \
    TF_NS::Tf_PostWarningHelper(TF_CALL_CONTEXT, __VA_ARGS__)

#ifdef TF_STATUS
#undef TF_STATUS
#endif // TF_STATUS
#define TF_STATUS(...)                              \
    TF_NS::Tf_PostStatusHelper(TF_CALL_CONTEXT, __VA_ARGS__)

#ifdef TF_ERROR
#undef TF_ERROR
#endif // TF_ERROR
#define TF_ERROR(...)                              \
    TF_NS::Tf_PostErrorHelper(TF_CALL_CONTEXT, __VA_ARGS__)

#ifdef TF_QUIET_ERROR
#undef TF_QUIET_ERROR
#endif // TF_ERROR
#define TF_QUIET_ERROR(...)                              \
    TF_NS::Tf_PostQuietlyErrorHelper(TF_CALL_CONTEXT, __VA_ARGS__)

// See documentation above.
#define TF_VERIFY(cond, ...)                                                   \
    (ARCH_LIKELY(cond) ? true :                                                \
    TF_NS::Tf_FailedVerifyHelper(TF_CALL_CONTEXT, # cond,                      \
                           TF_NS::Tf_VerifyStringFormat(__VA_ARGS__)))
